<script setup lang="ts">
  ...
  import type { SearchDetailResponse } from '../../types/api'
  ...
  const userInteractionDetail = ref<SearchDetailResponse | null>(null)
  ...
  const toggleFollow = async () => {
    if (followingActionLoading.value || !userInteractionDetail.value) return
    followingActionLoading.value = true
    try {
      const response = await userAPI.followUser(userId.value)
      if (response.success) {
        // 切换关注状态
        userInteractionDetail.value.isFollowing = !userInteractionDetail.value.isFollowing
        // 更新粉丝数
        userInteractionDetail.value.followerCount = userInteractionDetail.value.isFollowing
          ? userInteractionDetail.value.followerCount + 1
          : Math.max(0, userInteractionDetail.value.followerCount - 1)
      }
    } catch (err) {
  ...
  const loadUserData = async () => {
    try {
      loading.value = true
      const [interactionResponse, publicDetailResponse] = await Promise.all([
        userAPI.getInteractionDetail(userId.value),
        userAPI.getUserPublicDetail(userId.value),
      ])
      if (interactionResponse.success && interactionResponse.data) {
        userInteractionDetail.value = interactionResponse.data
      }
      if (publicDetailResponse.success && publicDetailResponse.data) {
        userPublicDetail.value = publicDetailResponse.data
      }
      await loadUserPosts()
    } catch (err) {
      ...
      <div class="header-content">
        <div class="stats-container">
          <div class="stat-item">
            ...
            <span class="stat-value">{{ userInteractionDetail?.followeeCount || 0 }}</span>
          </div>
          <div class="stat-item">
            <span class="stat-label">粉丝</span>
            <span class="stat-value">{{ userInteractionDetail?.followerCount || 0 }}</span>
          </div>
        </div>
        <div class="user-info-container">
          <div class="user-name-container">
            <h1 class="user-name">{{ userInteractionDetail?.userName || '用户' }}</h1>
            <button
              class="follow-button"
              :class="{ following: userInteractionDetail?.isFollowing }"
              :disabled="followingActionLoading"
              @click="toggleFollow"
            >
              {{
                followingActionLoading
                  ? '处理中...'
                  : userInteractionDetail?.isFollowing
                    ? '已关注'
                    : '关注'
              }}
            </button>
          ...
          <div class="user-avatar-container">
            <img
              v-if="userInteractionDetail?.userAvatar"
              :src="processImageData(userInteractionDetail.userAvatar)"
              alt="头像"
              class="user-avatar"
            />
            <div v-else class="user-avatar-placeholder">
              {{ userInteractionDetail?.userName?.charAt(0)?.toUpperCase() || 'U' }}
            </div>
          </div>
        </div>
      </div>
